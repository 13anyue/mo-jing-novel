...
function bind(){const rt=window.NovelRuntime,input=$('rt_input');const act=a=>{try{if(a==='mode')return rt.toggleNovelMode?.();if(a==='save')return rt.saveGame?.();if(a==='saves')return rt.showSaves?.();if(a==='fullscreen')return rt.toggleFullscreen?.();if(a==='choice')return rt.requestChoices?.();if(a==='continue')return rt.continueStory?.();if(a==='rewrite')return rt.rewriteLast?.();if(a==='settings'){const x=$('v7Settings');if(x)x.hidden=false;return}if(a==='cast')return window.App?.go?.('group-chat')||window.App?.go?.('npc');if(a==='map')return window.App?.go?.('map');if(a==='history')return rt.showHistory?.();if(a==='more')return window.MOPlatformV6?.more?.()}catch(e){safe(()=>window.App?.toast?.('操作失败：'+(e.message||e)),null)}};
document.querySelectorAll('[data-v7-action]').forEach(b=>b.addEventListener('click',()=>act(b.dataset.v7Action)));document.querySelector('[data-v7-close]')?.addEventListener('click',()=>{$('v7Settings').hidden=true});document.querySelector('[data-v7-back]')?.addEventListener('click',()=>safe(()=>window.App?.back?.()||history.back(),null));$('v7Send')?.addEventListener('click',()=>rt.playerSend?.());input?.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();rt.playerSend?.()}});input?.addEventListener('input',()=>{input.style.height='auto';input.style.height=Math.min(input.scrollHeight,120)+'px'})}
function install(){installPromptBridge();const rt=window.NovelRuntime;if(!rt)return;rt.__v7Render=render;rt.init=function(){render()};const old=rt.onEnter;rt.onEnter=function(){safe(()=>old?.call(this),null);render()};if($('page-runtime')?.classList.contains('active'))render()}
