...
function enhance(page){if(!page||page.dataset.mo5Enhanced==='1')return;const id=(page.id||'').replace(/^page-/,'');page.dataset.pageId=id;if(id==='home'||id==='runtime')return;const title=label[id]||page.querySelector('h1,h2,.page-title')?.textContent?.trim();if(!title)return;const b=document.createElement('button');b.className='mo5-page-mark mo5-back';b.type='button';b.innerHTML='‹ 返回';b.onclick=back;page.appendChild(b);page.classList.add('mo-v5-page');page.dataset.mo5Enhanced='1'}
function scan(){document.querySelectorAll('.page-view').forEach(enhance)}
