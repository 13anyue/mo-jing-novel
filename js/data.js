...
/* v34 loader：统一视觉文游平台 + 运行时 QA。业务模块按原顺序加载，最后由平台层接管视觉与导航。 */
(function(){
  ...
  load('js/material-library.js').then(()=>load('js/ui-overhaul.js')).then(()=>load('js/platform-upgrade.js')).then(()=>load('js/interaction-safety.js')).then(()=>load('js/memory-autocapture.js')).then(()=>load('js/ui-reference.js')).then(()=>load('js/ui-scene-pages.js')).then(()=>load('js/world-simulation.js')).then(()=>load('js/world-simulation-v2.js')).then(()=>load('js/world-state-v3.js')).then(()=>load('js/ui-v3.js')).then(()=>load('js/vn-engine-v2.js')).then(()=>load('js/ui-platform-v5.js')).then(()=>load('js/platform-v2.js')).then(()=>load('js/platform-v2-fix.js')).then(()=>load('js/platform-v6.js')).then(()=>load('js/platform-v6-qa.js')).catch(e=>console.warn('[墨境平台] 模块加载失败：',e));
})();
