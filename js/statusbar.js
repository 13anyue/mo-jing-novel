...
const StatusBar = {
  ...
  renderPage() {
    ...
    page.innerHTML = `
      ...
      <div class="grid grid-2">
        ...
        <div class="card"><div class="card-header"><h3>预览</h3></div><div class="card-body" id="statusPreviewArea"></div></div>
      </div>
      <div style="display:flex;gap:8px;margin-top:var(--space-lg);">
        <button class="btn btn-primary" onclick="StatusBar.save()">保存</button>
        <button class="btn btn-secondary" onclick="StatusBar.addField()">➕ 状态项</button>
        <button class="btn btn-secondary" onclick="StatusBar.addAffection()">➕ 好感度</button>
      </div>
    `;
  ...
  renderCustomFields() {
    ...
    c.innerHTML = cfg.customFields.map(f => `
      <div class="regex-item" style="margin-bottom:8px;">
        ...
        <button class="btn btn-sm btn-danger" onclick="StatusBar.removeField('${f.id}')">删除</button>
      </div>
    `).join('');
  },
  ...
  renderAffection() {
    ...
    c.innerHTML = cfg.npcAffection.map(a => {
      ...
      return `<div class="regex-item" style="margin-bottom:8px;">
        ...
        <button class="btn btn-sm btn-danger" onclick="StatusBar.removeAff('${a.id}')">删除</button>
      </div>`;
    }).join('');
  },
  ...
  getHTML() {
    const c = this.getConfig();
    let h = '';
    if (c.showScene) h += `<div class="status-row"><span class="status-label">📍 场景</span><span class="status-value" id="st_sceneName">${Storage.get('currentScene', '未知')}</span></div>`;
    if (c.showTime) h += `<div class="status-row"><span class="status-label">⏰ 时间</span><span class="status-value" id="st_timeVal">--:--</span></div>`;
    if (c.showWorld) { const w = Storage.get('worldData', {}); if (w.name) h += `<div class="status-row"><span class="status-label">🌍 世界</span><span class="status-value">${w.name}</span></div>`; }
    ...
    c.npcAffection.forEach(a => {
      ...
      h += `<div class="status-row" style="flex-direction:column;align-items:stretch;"><div style="display:flex;justify-content:space-between;"><span class="status-label">💖 ${name}</span><span class="status-value">${a.value}${a.max ? '/' + a.max : ''}</span></div><div class="affection-bar"><div class="affection-fill" style="width:${pct}%"></div></div></div>`;
    });
  ...
  getOverlayHTML() {
    const c = this.getConfig();
    let h = '';
    if (c.showScene) { const scene = Storage.get('currentScene', '--'); h += `<div>📍 ${scene}</div>`; }
    c.customFields.filter(f => f.type === 'text').forEach(f => { h += `<div>${f.label}: ${f.value}</div>`; });
    c.npcAffection.slice(0, 3).forEach(a => { const npc = (NPCManager?.getNPCs?.() || []).find(n => n.id === a.npcId); h += `<div>💖${npc?.name || '?'}: ${a.value}${a.max ? '/' + a.max : ''}</div>`; });
    return h;
  },
